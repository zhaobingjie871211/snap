...
#   t="snap_peek 0x20        ";     r=$($t|grep ']'|awk '{print $2}');echo -e "$t result=$r # Lockreg 0x1=locked"
    t="snap_peek 0x30        ";     r=$($t|grep ']'|awk '{print $2}');echo -e "$t result=$r # capabilityreg bit39..36=xfer_size bit35..32=DMA_align bit31..16=DRAM_size bit8=NVMe bit7..0=card type"
    xfer=$(( 2**${r:6:1} ))
    dma=$((  2**${r:7:1} ))
    dram=$(( 16#${r:8:4} ))
    nvme=${r:13:1}
